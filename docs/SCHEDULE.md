...
그리고 API 하루 요청 한도(10,000회)가 **한국시간 자정**에 초기화됩니다.
그 직후에 돌면 하루치를 온전히 씁니다.

### 시간대를 crontab 에 못 박아 둔 이유

서버 시간대는 이사하면 바뀝니다. Contabo 로 옮기고 나서 서버가
`Europe/Berlin` 이 되는 바람에, `10 15 * * *` 이 00:10 KST 가 아니라
**22:10 KST** 로 밀려 있었습니다. 에러가 안 나니 한참을 몰랐을 것입니다.

그래서 `CRON_TZ=Asia/Seoul` 을 적어 둡니다. 서버가 어디에 있든
한국시간 00:10 에 돕니다.
...
| | |
|---|---|
| 한도 | 10,000회 / 일 (한국시간 자정 초기화) |
| 일일 수집 | 254개 지역 × 3개월 = **762회** (7.6%) |
| 5년 백필 | 254 × 60 = **15,240회** → 이틀에 나눠야 함 |

한도를 넘기면 `[22] 일일 서비스 요청제한 횟수 초과` 가 옵니다.
이건 **재시도해도 절대 안 됩니다.** 그래서 수집이 즉시 멈추고,
어디까지 받았는지 로그에 남깁니다. `latest` 는 건드리지 않습니다.

지역 코드를 확인하느라 수동으로 API 를 많이 부른 날에는 그날 수집이
한도에 걸릴 수 있습니다. 다음 날 자동으로 회복됩니다.
