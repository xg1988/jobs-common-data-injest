...
# ---- 3. DB 반영 -------------------------------------------------------------
# DB 가 이 서버에 있을 때만 합니다 (docs/VPS_DB.md).
#
# Supabase 를 쓰던 때는 DB 가 미러에서 스스로 당겨왔습니다. DB 가 옆방에
# 있으면 그럴 이유가 없습니다 -- 바로 씁니다.
#
# 설정이 없으면 건너뜁니다. `ingest sync` 는 설정이 없으면 종료 코드 1 로
# 죽는데, 그걸 그대로 두면 DB 없이 파일만 쓰는 서버에서 매일 실패로 보입니다.
if "$PY" -c 'import sys; from ingest import db; sys.exit(0 if db.enabled() else 1)'; then
  "$PY" -m ingest sync --all
  echo "DB 반영 종료 코드: $?"
else
  echo "DB 설정이 없어 sync 를 건너뜁니다 (파일만 저장)"
fi
