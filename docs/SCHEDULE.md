...
01:30 KST   GitHub Actions    감시견 -- 어젯밤 수집이 제대로 됐나
11:00 KST   GitHub Actions    감시견 (한 번 더)

월 1회      사람이 직접        아카이브 -- 12개월 넘은 달을 DB 에서 파일로 (8장)
```
...
#### 레코드 수 급변 — 범위를 넓혔을 때는 정상입니다

조회 범위를 직접 넓히면 레코드가 몇 배로 뜁니다. 그건 사고가 아닙니다.
그래서 봉투에 조회 범위(`scope`)를 남기고, 다음 실행이 그걸 비교합니다.

| 이전 범위 | 판단 |
|---|---|
| 있고, 같다 | 에러 (API 가 조용히 깨진 신호) |
| 있고, 다르다 | 경고 — 통과시킵니다 |
| **없다 (모른다)** | 에러. 단, `--accept-scope-change` 로 다시 실행하라고 알려 줍니다 |

세 번째가 2026-08-21 전국 전환에서 실제로 걸린 경우입니다. `scope` 필드가
급변 검사보다 나중에 생겨서, 그때 남아 있던 `latest` 에는 지문이 없었습니다.
'모른다' 를 '같다' 로 읽는 바람에 103,407건이 통째로 격리됐습니다.

범위를 사람이 직접 바꿨을 때는 사람이 말해 줍니다. 코드가 추측하지 않습니다.
...
| 검사 | 범위 |
|---|---|
| `price_manwon` | 100 ~ 10,000,000 (100만 ~ 1000억) |
| `area_m2` | 10 ~ 500 |
| `floor` | -5 ~ 100 |
| `deal_date` | 미래 불가, 조회한 월 집합 안 |

하한이 100만원인 이유는 **싼 것과 깨진 것이 다르기** 때문입니다. 서울만 볼
때는 1천만원이 안전한 하한이었지만, 전국에서는 동해시 22㎡ 직거래 800만원처럼
실제로 있는 가격이 걸립니다. 1천만원 미만은 격리하지 않고 **세어서 경고**만
냅니다 (`저가 거래 N건`).
...
|---|---|
| `removed` 급증 | 전체의 5% 초과 |
| `_key` 충돌 | 카운트만 기록 (서울 1.8% / 전국 2.5%, 정상) |
| 저가 거래 | `price_manwon` 1,000만원 미만 — 카운트만 |
| 0건인 지역 | 조회한 모든 달에서 0건 — 행정구역 개편 의심 |
...
| cron | `30 16 * * *` (UTC) = **매일 01:30 KST** — 수집·DB 갱신 직후 |
| cron | `0 2 * * *` (UTC) = **매일 11:00 KST** — 아침에 한 번 더 |

**두 번 도는 이유.** 한 번만 돌면 밤에 난 사고를 반나절 뒤에 압니다.
2026-08-21 전국 전환이 격리됐을 때가 그랬습니다 -- 00:10 에 실패했는데
알림은 11:00 이었습니다. 01:30 은 DB 당겨오기(00:40)까지 끝난 뒤라
파일과 DB 를 함께 봅니다. 11:00 은 사람이 깨어 있는 시간에 한 번 더
확인하는 용도입니다 (밤 사이 메일을 못 봤을 수 있으니).
...
## 7-1. 아카이브 (월 1회) — 유일하게 자동화하지 않은 일

DB 에는 최근 12개월만 둡니다. 그보다 오래된 달은 파일로 내보내고 DB 에서
비웁니다. 안 하면 **전국 기준 19개월에 무료 한도 500 MB 가 찹니다.**

```bash
python -m ingest archive --source molit_apt_trade --hot-months 12          # 파일만 (안전)
python -m ingest archive --source molit_apt_trade --hot-months 12 --evict  # DB 도 비움
```

절차와 안전장치는 [ARCHIVE.md](ARCHIVE.md) 에 있습니다. 요약하면, 파일을 쓴 뒤
**다시 읽어서 행 수와 sha256 을 대조하고, 맞을 때만** 지웁니다.

### 왜 cron 에 안 넣었나

`--evict` 는 DB 에서 행을 지웁니다. 그러려면 `SUPABASE_SERVICE_ROLE_KEY` 가
필요합니다. 그 키를 VPS 에 두는 순간 이 설계의 핵심이 무너집니다 —
**"VPS 가 털려도 DB 를 지울 수는 없다"** (6장).

한 달에 한 번 5분이면 되는 일 때문에 그걸 포기할 이유가 없습니다.
그래서 **사람이 자기 컴퓨터에서** 돌립니다. 잊지 않도록 감시견이 아카이브가
밀리면 경고를 냅니다 (5장).

돌린 뒤에는 `data/archive/` 를 커밋해서 올려야 합니다 — 그게 그 달의 유일한
사본이 됩니다.
